...
char* generate_rand_ip6(char ip_address[INET6_ADDRSTRLEN]){
	int hi_num=15;
	int low_num=0;
  int count=0;
  for (int i = 0; i < 39; i++)
  {
    int  ran = (rand() % (hi_num - low_num)) + low_num;
    if(count==4){
      ip_address[i]=':';
      count=0;
    }else{
      count++;
      ip_address[i]=hex_to_char(ran);
    }
  } 
  return ip_address;    
}
int main (int argc, char *argv[]) {
  ...
  char interface[INET6_ADDRSTRLEN]={0};
  struct ip6_hdr iphdr;
  struct udphdr udphdr;
  char data[IP_MAXPACKET]={0};
  char src_mac[6]={0};
  char dst_mac[6]={0};
  int dst_port=443;
  char ip_address_dst[INET6_ADDRSTRLEN];
  char ether_frame[IP_MAXPACKET]={0};
  struct addrinfo hints, *res;
  struct sockaddr_in6 *ipv6;
  struct sockaddr_ll device;
  struct ifreq ifr;
  void *tmp;
  strcpy(ip_address_dst,"::1");
  for (int i = 1; i < argc; i++)
  {
    if(!strcmp(argv[i],"-t")){
      strcpy(ip_address_dst,argv[i+1]);
      i++;
